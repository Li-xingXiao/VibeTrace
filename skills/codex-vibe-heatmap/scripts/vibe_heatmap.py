...
def update_readme_block(
    *,
    ...
    block = "\n".join(
        [
            MARKER_START,
            f"### AI Vibe Heatmap ({source}, {year})",
            f"- Intensity mode: {intensity_mode}",
            f"- Active days: {int(summary['active_days'])}",
            f"- Sessions: {int(summary['sessions'])}",
            f"- Active minutes: {int(summary['active_minutes'])}",
            f"- Avg minutes/active day: {summary['avg_minutes_per_active_day']}",
            f"![Vibe Heatmap]({svg_ref})",
